...
class TrainSet(Dataset):
    def __init__(self, source_file, opt=None):
        ...
        with open(source_file, 'r') as f:
            ...
            for i in tqdm(range(i_range),ncols=100):
                data = next(reader)[0].split('\t')
                
                # build inputs and targets
                if opt.DATASET_NORMALIZATION:
                ...
                try:
                    temp_create_time = datetime.datetime.strptime(data[3], "%Y-%m-%d %H:%M:%S")
                    temp_payed_time = datetime.datetime.strptime(data[4], "%Y-%m-%d %H:%M:%S")
                    temp_presell_time = datetime.datetime.strptime(data[9], "%Y-%m-%d %H:%M:%S")
                ...
                temp_input.append(float(temp_create_time.day))
                temp_input.append(float(temp_create_time.hour))
                temp_input.append(float(temp_payed_time.day))
                temp_input.append(float(temp_payed_time.hour))
    ...
    def __getitem__(self, idx):
        ...
        return (inputs, targets_sign_day, targets_sign_hour, targets_ship_day, targets_ship_hour, targets_got_day, targets_got_hour, targets_dlved_day, targets_dlved_hour,payed_time,signed_time)
...
#                 temp_presell_time = datetime.datetime.strptime(data[9], "%Y-%m-%d %H:%M:%S")
                ...
                temp_input.append(float(temp_payed_time.day))
                temp_input.append(float(temp_payed_time.hour))
#                 temp_input.append(float(temp_presell_time.day))
#                 temp_input.append(float(temp_presell_time.hour))
